...
def upgrade():
    ...
    op.create_index(
        op.f("ix_external_applications_token_hash"),
        "external_applications",
        ["token_hash"],
        unique=False,
    )
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM("Active", "Restricted", name="userstatus"),
            nullable=True,
        ),
...
def downgrade():
    ...
    op.drop_table("users")
    op.drop_index(
        op.f("ix_external_applications_token_hash"), table_name="external_applications"
    )
